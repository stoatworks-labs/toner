...
test card and reports any that made no difference at all.

    python3 tools/sweep.py [--binary build/totest] [--size WxH] [--jobs N] [--allow-no-gl]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**The card has to have greys, solids and a background.** It is light-grey
paper (so Bg Suppression has a background to throw away), a photograph-like
gradient (so Contrast has greys to collapse), a big solid and a row of
smaller ones (so Solid Fill and Toner Supply have coverage to work on), thin
rules and text-like marks, and a moving bar. `totest --out` renders it.
...
**An option's range is its element count, an integer's range is real.** An
option parameter's range reads back 0..1 whatever its element count, so
`totest --list` prints the count as the range, and the ends swept are the
first and last element; Generations lists 1..8 and is swept at both.
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "totest"))
    ...
    scratch = tempfile.mkdtemp(prefix="tosweep")

    # One render first: a runner with no GL at all cannot sweep anything, and
    # that has to read as a skip, loudly, not as seventeen dead controls.

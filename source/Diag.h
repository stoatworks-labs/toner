...
/**
    ...
    **A shader that will not compile.** `InitGL` returns `FF_FAIL` and from
    ...
    message anywhere; with twelve passes this also records *which* one, which
    is the difference between a five-minute fix and an afternoon. The GL vendor
    ...
    **A buffer that could not be allocated.** This plugin holds five page-sized
    buffers (four R32F, one RG32F) plus three small ones, and the failure mode
    when the driver says no is a black frame with nothing to explain it. What
    goes in the log is the size asked for: at 4K the five come to about 200 MB.
*/
namespace tonerfx::diag
{
